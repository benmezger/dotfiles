...
_boot-lenin() {
  ...
  until ssh benmezger@"$router_ip" "ping -c1 -W1 $machine_ip" &>/dev/null; do
    echo "Not up yet, retrying..."
    sleep 5
  done

  echo "SSHing in for LUKS crypt..."
  ssh -A -t benmezger@"$router_ip" "ssh root@$machine_ip"

  echo "Waiting for machine's sshd..."
  until ssh -o ConnectTimeout=5 -o BatchMode=yes \
	    "$final_ssh_target" echo ok 2>/dev/null; do
    echo "Not up yet..."
    sleep 5
  done
