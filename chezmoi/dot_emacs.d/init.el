  ...
  (let ((capture-dir (concat user-emacs-directory "org-captures/")))
    (setq org-capture-templates
      `(("c" "Code" entry (file ,(concat org-directory "refs/code.org"))
         ...
         ("i" "Inbox" entry (file+olp+datetree ,(concat org-directory "refs/inbox.org.gpg"))
           (file ,(concat capture-dir "inbox-snippet.capture")))
         ("b" "Blog post" entry (file+olp ,(concat org-directory "refs/blog.org") "Posts")
           (file ,(concat capture-dir "blog-post.capture")))
         ("n" "Note" entry (file+olp ,(concat org-directory "refs/notes.org.gpg") "Inbox")
           "* %?\nEntered on %U\n  %i\n  %a")
         ("t" "Todo" entry (file ,(concat org-directory "refs/todos.org.gpg"))
           "* TODO %?\n %i\n  %a")
         ("r" "Register new book" entry (file+olp ,(concat org-directory "refs/notes.org.gpg") "Books")
           (file ,(concat capture-dir "new-book.capture")))
         ("d" "Decision note" entry (file ,(concat org-directory "refs/decisions.org.gpg"))
           (file ,(concat capture-dir "decision.capture")))
         ("w" "Work TODO" entry (file+olp ,(concat org-directory "work/ah.org.gpg") "Tasks")
